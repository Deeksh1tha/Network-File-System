...
int ReadFile(int sockid, char *token, char *return_buffer)
{
    ...
    {
        perror("fopen");
        strcpy(return_buffer, "File unavailable\n");
        return -1;
    }
    printf("helli\n");
    struct TrieNode *node = GetTrieNode(trie_root, token);
    ...
    fgets(return_buffer, MAX_FILE_LENGTH, fp);
    printf("hoii %s\n", return_buffer);
...
int WriteFile(int sockid, char *token, char *return_buffer)
{
    ...
    if (fputs(write_buffer, fp) == EOF)
    {
        ...
        strcpy(return_buffer, "Error writing to file\n");
        fclose(fp);
        return -1;
    }
...
{
    strcpy(return_buffer, "");
    receiveChunks(sockid, return_buffer);
    ...
    fwrite(return_buffer, sizeof(char), strlen(return_buffer), fp);
...
void *client_handler(void *arg)
{
    ...
    if (client->type) // client->type=1 ==> nm
    {
        if (strcmp(operation, "CREATE") == 0)
        {
            int response = CreateFileDirectory(strtok(NULL, "\t\n"), return_buffer);
        }
        else if (strcmp(operation, "DELETE") == 0)
        {
            int response = DeleteFileDirectory(strtok(NULL, "\n\t"), return_buffer);
        }
        else if (strcmp(operation, "GIVEFILES") == 0)
        {
            int response = getFilesInDir(strtok(NULL, "\t\n"), return_buffer);
        }
        else if (strcmp(operation, "GET") == 0)
        {
            int response = ReadFile(client->sockid, strtok(NULL, "\t\n"), return_buffer);
            printf("ret: %s\n", return_buffer);
        }
        else if (strcmp(operation, "PUT") == 0)
        {
            int response = PutFile(client->sockid, strtok(NULL, "\t"), return_buffer);
        }
